...
CPU_USAGE_RATE=`cat /proc/stat | grep 'cpu ' | sed 's/cpu  //g' | awk  '{split($0,t," "); for(i=NF;i>0;i--) s = s + $i } END {print 1 - ($4/s) }'`
CPU_USAGE_PERC=`bc <<< "scale=2; ($CPU_USAGE_RATE * 100)"`
...
echo -e "${WHITE}CPU usage              :${GREEN}$CPU_USAGE_PERC %"
